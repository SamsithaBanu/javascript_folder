...
const displayUI = function(currentAccount){
  displayMovements(currentAccount);
    ...
    displayCalcAverage(currentAccount);
...
const displayCalcAverage = function(account){
  account.balance = account.movements.reduce((acc,cur,i)=>acc+cur,0);
  labelBalance.textContent = formatCur(account.balance, account.locale, account.currency);
}

const displaySummary = function(acc){
  const summaryIn = acc.movements.filter(mov=> mov > 0).reduce((acc,mov)=>acc+mov,0);
  labelSumIn.textContent = formatCur(summaryIn, acc.locale, acc.currency);

  const summaryOut = acc.movements.filter(mov=> mov < 0).reduce((acc,mov)=>acc+mov,0);
  ...
  .reduce((acc,intres)=>acc+intres,0);
